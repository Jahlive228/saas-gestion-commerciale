...
export default function UserDropdown() {
  ...
  const userName = user?.user
    ? `${user.user.first_name || ""} ${user.user.last_name || ""}`.trim() || "Utilisateur"
    : "Utilisateur";
  const userEmail = user?.user?.email || "email@example.com";
  const userRole = user?.jwtPayload?.role_name || "Utilisateur";
    ...
    <div className="relative">
      ...
      >
        <div>
          ...
          <span className="mt-1 inline-block px-2 py-0.5 text-xs font-medium bg-brand-50 text-brand-700 rounded-full">
            {formatRole(userRole)}
          </span>
        </div>
